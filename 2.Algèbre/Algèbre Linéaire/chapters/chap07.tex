...
\vspace{0.7cm}
In summary, the \emph{determinant}, \emph{rank}, and \emph{trace} of a matrix are invariants\footnote{
    And also include the \emph{characteristic polynomial}, \emph{eigenvalues}, and \emph{eigenvectors} 
    in next section.
...
\begin{definition}{Eigenspace}
    ...
    the set of all eigenvectors corresponding to \( \lambda \) \emph{along with the zero vector}:
    \[
...
\begin{lemma}
    ...
    are the corresponding \emph{linearly independent} eigenvectors of \( \lambda_i \),
...
\begin{theorem}
    ...
    \begin{enumerate}
        ...
        \item \( V = V_{\lambda_1} \oplus V_{\lambda_2} \oplus \cdots \oplus V_{\lambda_k} \),
            where \( \lambda_1, \lambda_2, \dots, \lambda_k \) are the \emph{distinct} eigenvalues of \( \mathcal{A} \).
        \item The minimal polynomial\footnote{
            ...
            i.e. it can be factored into \emph{distinct} linear factors over field \( F \):
            \[
...
Let \(V\) be a linear space over field \( F \) (without specific statements, it can be \emph{infinite} dimensional),
and let \( \mathcal{A}\in \operatorname{Hom}(V) \).
...
\begin{enumerate}
    ...
    \item \(A\) is diagonalizable. Furthermore, it can be unitarily diagonalized by the \emph{same} DFT matrix.
\end{enumerate}
