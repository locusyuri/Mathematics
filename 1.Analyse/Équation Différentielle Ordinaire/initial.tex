...
\begin{theorem}
    ...
    \Phi(x) =
    \begin{pmatrix}
        e^{\lambda_{1} x} \mathbf{\xi}_{1} & e^{\lambda_{2} x} \mathbf{\xi}_{2} & \cdots & e^{\lambda_{n} x} \mathbf{\xi}_{n}
    \end{pmatrix},
    \]
    where \(\mathbf{\xi}_{i}\) is the eigenvector corresponding to eigenvalue \(\lambda_{i}\) of 
...
\begin{equation}\label{eq:solution form for repeated eigenvalue}
    ...
    \left[ \xi_{0} + \frac{x}{1!}\xi_{1} + \frac{x^2}{2!}\xi_{2} + \cdots + \frac{x^{n_j - 1}}{(n_j - 1)!}\xi_{n_j - 1} \right],
\end{equation}
...
\begin{lemma}
    ...
    if and only if
    \(\xi_{0}\) is a non-zero solution to
    \begin{equation}\label{eq:generalized eigenvector equation}
        (\mathbf{A} - \lambda_{j} \mathbf{E})^{n_{j}} \xi_{0} = 0,
    \end{equation}
    and \(\xi_{1}, \xi_{2}, \ldots, \xi_{n_j - 1}\) satisfy the following chain of equations:
    \begin{equation}\label{eq:generalized eigenvector chain}    
        \begin{cases}
            \xi_{1} = (\mathbf{A} - \lambda_{j} \mathbf{E}) \xi_{0}, \\
            \xi_{2} = (\mathbf{A} - \lambda_{j} \mathbf{E})^{2} \xi_{1}, \\
            \vdots \\
            \xi_{n_j - 1} = (\mathbf{A} - \lambda_{j} \mathbf{E})^{n_{j} - 1} \xi_{0}.
        \end{cases}
    \end{equation}
\end{lemma}
...
\begin{theorem}
    ...
    \[
    \mathbf{P}_{j}^{(j)}(x) =
        \xi_{j0}^{(i)} + \frac{x}{1!} \xi_{j1}^{(i)} + \frac{x^{2}}{2!} \xi_{j2}^{(i)} + 
        \cdots + \frac{x^{n_j - 1}}{(n_j - 1)!} \xi_{j, n_j - 1}^{(i)},
    \]
    which is the \(j\)-th vector polynomial corresponding to eigenvalue \(\lambda_{i}\) 
    (\(i = 1, 2, \ldots, n_{j};j = 1, 2, \ldots, n_{i}\)).
    \(\xi_{10}^{(i)}, \cdots, \xi_{n_{i}0}^{(i)}\) is \(n_{i}\) linearly independent solutions to
    ~\eqref{eq:generalized eigenvector equation},
    and the other \(\xi_{jl}^{(i)}\) (\(j = 1, 2, \ldots, n_{i}; l = 1, 2, \ldots, n_{j} - 1\))
    is obtained by replacing corresponding \(\xi_{j0}^{(i)}\) with \(\xi_{0}\) in~\eqref{eq:generalized eigenvector chain}.
\end{theorem}
