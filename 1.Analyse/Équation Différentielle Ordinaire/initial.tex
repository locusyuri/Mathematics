...
\chapter{Qualitative and Stability Theory} % 定性与稳定性理论
...
Let the unique solution to~\eqref{eq:autonomous system Lyapunov}
with initial condition \(t_{0}, \mathbf{x}_{1}\) be denoted as \(\mathbf{x}= \boldsymbol{\phi}(t, t_{0}, \mathbf{x}_{1})\).
The other solutions are denoted as \(\mathbf{x} = \mathbf{x}(t, t_{0}, \mathbf{x}_{0})\).

\begin{definition}{Stability in the Sense of Ляпунов}
    The solution \(\mathbf{x} = \boldsymbol{\phi}(t, t_{0}, \mathbf{x}_{1})\) to~\eqref{eq:autonomous system Lyapunov}
    ...
    such that when \(\|\mathbf{x}_{0} - \mathbf{x}_{1}\| < \delta\),
    it holds that
    \[
    \|\mathbf{x}(t, t_{0}, \mathbf{x}_{0}) - \boldsymbol{\phi}(t, t_{0}, \mathbf{x}_{1})\| < \varepsilon,
    \]
    for all \(t \geqslant  t_{0}\).

    If \(\mathbf{x} = \boldsymbol{\phi}(t, t_{0}, \mathbf{x}_{1})\) is stable, 
    and there exists \(\delta_{1}\in (0, \delta]\) such that when \(\|\mathbf{x}_{0} - \mathbf{x}_{1}\| < \delta_{1}\),
    it holds that
    \[
    \lim_{t \to +\infty} \|\mathbf{x}(t, t_{0}, \mathbf{x}_{0}) - \boldsymbol{\phi}(t, t_{0}, \mathbf{x}_{1})\| = 0,
    \]
    then \(\mathbf{x} = \boldsymbol{\phi}(t, t_{0}, \mathbf{x}_{1})\) is said to be \textbf{asymptotically stable}.

    If \(\mathbf{x} = \boldsymbol{\phi}(t, t_{0}, \mathbf{x}_{1})\) is asymptotically stable,
    and there exists \(\delta_{2}\in (0, \delta_{1}], \alpha, \beta\) such that 
    when \(\|\mathbf{x}_{0} - \mathbf{x}_{1}\| < \delta_{2}\),
    it holds that
    \[
    \|\mathbf{x}(t, t_{0}, \mathbf{x}_{0}) - \boldsymbol{\phi}(t, t_{0}, \mathbf{x}_{1})\| \leqslant 
    \alpha \|\mathbf{x}_{0} - \mathbf{x}_{1}\| e^{-\beta (t - t_{0})},
    \]
    for all \(t \geqslant t_{0}\),
    then \(\mathbf{x} = \boldsymbol{\phi}(t, t_{0}, \mathbf{x}_{1})\) is said to be \textbf{exponentially stable}.
\end{definition}

By substitution
\[
\mathbf{y} = \mathbf{x}(t, t_{0}, \mathbf{x}_{0}) - \boldsymbol{\phi}(t, t_{0}, \mathbf{x}_{1}),
\]
we have
\begin{align*}
    ...
    := & \mathbf{F}(t, \mathbf{y}),
\end{align*}
then the system~\eqref{eq:autonomous system Lyapunov} can be transformed into:
...
to~\eqref{eq:autonomous system Lyapunov} is \underline{equivalent to the stability of the zero solution}
...
to the autonomous system~\eqref{eq:autonomous system Lyapunov},
...
\begin{definition}{Stability of the Zero Solution}
    The zero solution \(\mathbf{x} = \mathbf{0}\) to~\eqref{eq:autonomous system Lyapunov}
...
\begin{leftbarTitle}{Lyapunov Second Method}\end{leftbarTitle} % Lyapunov 第二方法
...
\begin{equation}\label{eq:autonomous system Lyapunov 2}
...
\begin{definition}{Ляпунов Function}
    ...
    \end{itemize}
    If \(V(\mathbf{x}) > 0 (<0)\) for all \(\mathbf{x} \in G \setminus \{\mathbf{0}\}\),
    then \(V(\mathbf{x})\) is called a \textbf{positive/negative definite Ляпунов function};
    if \(V(\mathbf{x}) \geqslant 0 (\leqslant 0)\) for all \(\mathbf{x} \in G_{1} \subseteq G\),
    then \(V(\mathbf{x})\) is called a \textbf{positive/negative semi-definite Ляпунов function};
    otherwise, it is called an \textbf{indefinite Ляпунов function}.
\end{definition}
