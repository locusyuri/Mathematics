...
\begin{theorem}{Square Approximation Property of Fourier Series} % Fourier 级数的平方逼近性质
    Let \(f(x)\in \mathcal{R}[-\pi, \pi]\), and \(W\) be an \(N\)-degree subspace of \(\mathcal{R}[-\pi, \pi]\),
...
\begin{theorem}{Bessel's Inequality} % 贝塞尔不等式
    Let \( f(x) \in \mathcal{R}[-\pi, \pi] \) with Fourier coefficients \( \hat{f}(n) \),
...
\begin{theorem}{Parseval's Identity} % Parseval 恒等式
    ...
    \]
    where \( g(x) \in \mathcal{R}[-\pi, \pi] \) with Fourier coefficients \( \hat{g}(n) \),
    \[
