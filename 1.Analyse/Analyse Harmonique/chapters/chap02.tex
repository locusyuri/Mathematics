...
In the previous chapter, we denote \(\mathcal{R}[-\pi, \pi]\) as the linear space whose elements are
all Riemann integrable or absolutely Riemann integrable functions on \([-\pi, \pi]\).
Now, for convenience, we introduce the notation \(\mathcal{R}^{2}[-\pi, \pi]\) as the linear space whose elements are
...
we have
\[
\mathcal{R}^{2}[-\pi, \pi] \subset \mathcal{R}[-\pi, \pi].
\]
...
\section{Square mean Convergence} % 平方平均收敛
\begin{leftbarTitle}{Parseval's Identity}\end{leftbarTitle}
\begin{theorem}{Square Approximation Property of Fourier Series} % Fourier 级数的平方逼近性质
    Let \(f(x)\in \mathcal{R}^{2}[-\pi, \pi]\), and \(W\) be an \(N\)-degree subspace of \(\mathcal{R}^{2}[-\pi, \pi]\),
    then the best approximation of \(f(x)\) in \(W\) is just given by its Fourier series partial sum \(S_{N}(f; x)\):
    \[
    S_{N}(f; x) = \sum_{n=-N}^{N} \hat{f}(n) e^{inx} = \frac{a_0}{2} + \sum_{n=1}^{N} \left[ a_n \cos(nx) + b_n \sin(nx) \right].
    \]
    And the remainder \( E_{N}(f; x) = f(x) - S_{N}(f; x) \) satisfies\footnote{
        Refer to \emph{Algèbre Linéaire - Section 10.5: Orthogonal Completion and Orthogonal Projection}.
    }:
    \[
    \| E_{N}(f; x) \|^2 = \| f \|^2 - \sum_{n=-N}^{N} |\hat{f}(n)|^2
    = \frac{1}{\pi}\int_{-\pi}^{\pi} f^{2}(x) \, \mathrm{d}x - \left[ \frac{a_0^2}{2} + \sum_{n=1}^{N} (a_n^2 + b_n^2) \right].
    \]
\end{theorem}

\begin{theorem}{Bessel's Inequality} % 贝塞尔不等式
    Let \( f(x) \in \mathcal{R}^{2}[-\pi, \pi] \) with Fourier coefficients \( \hat{f}(n) \),
    then for any integer \( N \geqslant 0 \):
    \[
    \sum_{n=-N}^{N} |\hat{f}(n)|^2 \leqslant \| f \|^2,
    \]
    or in real form:
    \[
    \frac{a_0^2}{2} + \sum_{n=1}^{N} (a_n^2 + b_n^2) \leqslant \frac{1}{\pi} \int_{-\pi}^{\pi} f^{2}(x) \, \mathrm{d}x.
    \]
\end{theorem}

\begin{theorem}{Parseval's Identity} % Parseval 恒等式
    Let \( f(x) \in \mathcal{R}^{2}[-\pi, \pi] \) with Fourier coefficients \( \hat{f}(n) \),
    then:
    \[
    ...
    \[
    \sum_{n=-\infty}^{+\infty} \hat{f}(n) \overline{\hat{g}(n)} = \langle f, g \rangle,
    \]
    or in real form:
    \[
    \frac{a_0 c_0}{2} + \sum_{n=1}^{+\infty} (a_n c_n + b_n d_n) = \frac{1}{\pi} \int_{-\pi}^{\pi} f(x) g(x) \, \mathrm{d}x,
    \]
    where \( g(x) \in \mathcal{R}^{2}[-\pi, \pi] \) with Fourier coefficients \( \hat{g}(n) \),
    \[
...
\begin{theorem}{Wirtinger Inequality} % Wirtinger 不等式
    Let \( f(x)\in C^{1}[-\pi, \pi] \),
    and satisfies:
    \[
    \int_{-\pi}^{\pi} f(x) \, \mathrm{d}x = 0, f(-\pi) = f(\pi),
    \]
    then:
    \[
    \int_{-\pi}^{\pi} f^{2}(x) \, \mathrm{d}x \leqslant \int_{-\pi}^{\pi} f'^{2}(x) \, \mathrm{d}x,
    \]
    with equality if and only if \( f(x) = A \cos x + B \sin x \) for some constants \( A, B \in \mathbb{R} \).
\end{theorem}

\begin{theorem}{Poincaré Inequality} % Poincaré 不等式
\end{theorem}

\begin{theorem}{Friedrichs Inequality} % Friedrichs 不等式
    
\end{theorem}


\begin{leftbarTitle}{Square Mean Convergence}\end{leftbarTitle}
\begin{definition}{Square Mean Convergence}
    A sequence of functions \( f_{n}(x) \) is said to converge to \( f(x) \)
    in the square mean (or \( L^{2} \)) sense on interval \([a, b]\),
    if
    \[
    \lim_{n \to \infty} \| f_{n} - f \|^{2} = 
    \lim_{n \to \infty} \int_{a}^{b} | f_{n}(x) - f(x) |^{2} \, \mathrm{d}x = 0.
    \]
\end{definition}

\begin{theorem}
    If \(f(x)\in \mathcal{R}^{2}[-\pi, \pi]\),
    then its Fourier series converges to \(f(x)\) in the square mean sense on \([-\pi, \pi]\), i.e.,
    \[
        \lim_{N \to \infty} \int_{-\pi}^{\pi} | f(x) - S_{N}(f; x) |^{2} \, \mathrm{d}x = 0.
    \]
\end{theorem}
...
\begin{example}
    ...
    \begin{enumerate}
        ...
        \item \[\sum_{n=1}^{\infty} \frac{(-1)^{n+1}}{n^{4}} = \frac{7\pi^{4}}{720}; \]
        \item \[\sum_{n=1}^{\infty} \frac{1}{n^{4}} = \frac{\pi^{4}}{90}. \]
    \end{enumerate}
\end{example}
\begin{proof}
    
    \noindent{\color{violet!80}\textbf{1.}}{\color{violet!80}}
    By~\ref{ex:1},
    \[
    ...
    \[
    4 \sum_{n=1}^{\infty} \frac{1}{n^{2}} = \frac{1}{\pi} \int_{-\pi}^{\pi} x^{2} \, \mathrm{d}x = \frac{2\pi^{2}}{3},
    \]
    i.e.,
    \[
    ...
    \noindent{\color{violet!80}\textbf{2.}}{\color{violet!80}}
\end{proof}

\section{Equidistribution} % 等分布问题


\section{Poisson Kernel} % Poisson 核